...
def potential_customer_list(request):
    ...
    # ✅ Get pipelines created by this user (for pipeline modal)
    ...
    return render(request, 'accounts/potential_customers.html', {
        ...
        'pipelines': pipelines,  # <-- Add this to enable pipeline modal dropdown
    })
...
def convert_lead_to_customer(request, pk):
    ...
    Customer.objects.create(
        ...
        customer_surname="",  # Optional to prompt user later
        customer_email=lead.potential_customer_email,
        customer_phone=lead.potential_customer_phone,
        customer_company="",  # <- Leave company blank
        customer_address="",  # <- Leave address blank too
...
def ticket_detail(request, id):
    ...
    if request.method == 'POST':
        ...
        # 2) Save ticket (with stamps)
        ticket.save()

        # 3) Send resolution email if we just resolved it
        ...
        # 6) If it was just a field‐only update (no comment/email),
...
def mark_pipeline_entry_undone(request, entry_id):
    ...
    # If you also want to clear the “first done” marker:
...
def reports(request):
    ...
    if form.is_valid():
        ...
        # ----------------------------------------------------------------
        # 3) Pipeline Total Entries Added
        # ----------------------------------------------------------------
